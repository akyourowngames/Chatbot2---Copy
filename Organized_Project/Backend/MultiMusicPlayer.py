...
class MultiMusicPlayer:
    ...
    def search_spotify(self, query: str) -> Optional[Dict]:
        """
        Search Spotify and return embed URL.
        Uses the Spotify oEmbed endpoint (no API key needed).
        """
        try:
            # First, try to search via Spotify's web search
            # Since we don't have API access, we'll help users with direct links
            
            # Generate a Spotify search URL for the user
            search_url = f"https://open.spotify.com/search/{quote_plus(query)}"
            
            # For now, return a search link (actual embed requires track URL)
            return {
                "platform": "spotify",
                "type": "search",
                "search_url": search_url,
                "message": f"🎵 Search on Spotify: {search_url}",
                "note": "Paste a Spotify track/playlist URL for embedded playback"
            }
            
        except Exception as e:
            print(f"[MultiMusicPlayer] Spotify search error: {e}")
            return None
    
    def get_spotify_embed(self, url: str) -> Optional[Dict]:
        """Get Spotify embed from a track/album/playlist URL"""
        try:
            # Parse Spotify URL
            match = None
            for pattern in self.platform_patterns['spotify']:
            ...
            content_type = match.group(1)  # track, album, or playlist
            content_id = match.group(2)
            
            # Generate embed URL
            embed_url = f"https://open.spotify.com/embed/{content_type}/{content_id}?utm_source=generator&theme=0"
            
            # Try to get oEmbed info
            ...
            print(f"[MultiMusicPlayer] Spotify embed error: {e}")
            return None
    ...
    def play(self, query: str, source: str = "auto") -> Dict[str, Any]:
        ...
        Args:
            ...
            source: "auto", "youtube", "spotify", "soundcloud"
        ...
        if platform:
            # Direct URL provided
            if platform == 'spotify':
                result = self.get_spotify_embed(query)
                if result:
        ...
        if source == "spotify":
            result = self.search_spotify(clean_query)
            if result:
                return self._format_response(result, query)
        ...
        return {
            ...
            "suggestion": "Try a different search term or paste a direct URL from YouTube, Spotify, or SoundCloud."
        }
    ...
    def get_available_sources(self) -> List[Dict]:
        ...
        return [
            ...
            },
            {
                ...
                "name": "Spotify",
                "icon": "🎵",
                "description": "Play Spotify tracks (paste URL)",
...
if __name__ == "__main__":
    ...
    # Test Spotify URL
    print("\n=== Spotify URL ===")
    result = multi_music_player.play("https://open.spotify.com/track/4cOdK2wGLETKBW3PvgPWqT")
    print(json.dumps(result, indent=2))
