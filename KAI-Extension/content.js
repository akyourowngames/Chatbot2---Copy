...
function handleFillField(fieldHint, value) {
    ...
    for (const field of fields) {
        const label = field.label.toLowerCase();
        let score = 0;
        if (label === hint) score = 100;
        else if (label.includes(hint)) score = 50;
        else if (hint.split(' ').some(w => label.includes(w))) score = 25;
        if (score > bestScore) { bestScore = score; best = field; }
    }

    if (best) {
        fillField(best.element, value, { highlight: true });
        addMsg(`✅ Filled "${best.label}" with: ${value}`, 'success');
        speak(`Filled ${best.label}`);
    } else {
