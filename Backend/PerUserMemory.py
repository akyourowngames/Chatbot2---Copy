...
class PerUserMemorySystem:
    ...
    def add_memory(self, user_id: str, content: str, category: str = "general",
        ...
        if self.supabase:
            try:
                # Store embedding as JSON array (for pgvector, you'd use vector type)
                memory_data['embedding'] = json.dumps(embedding)
                self.supabase.table('user_memories').insert(memory_data).execute()
                logger.info(f"[MEMORY] Added for user {user_id[:8]}: {content[:50]}...")
                return memory_id
            except Exception as e:
                error_str = str(e).lower()
                if "does not exist" in error_str:
                    if self._create_memories_table():
                        return self.add_memory(user_id, content, category, importance, session_id, metadata)
