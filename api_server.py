...
Write in a professional, informative tone. Use clear paragraphs. Do NOT use markdown, bullets, or special formatting - just plain text paragraphs."""
                ...
                # For DocumentGenerator.generate_pdf(filename, content)
                 # content should be a list of sections (dicts), not a wrapper dict with title/subtitle
                 # because generate_pdf iterates over content to create the flow.
                 
                 pdf_path = document_generator.generate_pdf(topic.title(), sections)
                 # Handle return value from generate_pdf (url dict or filepath string)
                 if isinstance(pdf_path, dict):
                     cloud_url = pdf_path.get("url")
                     local_path = pdf_path.get("filepath", "")
                     pdf_filename = os_module.path.basename(local_path)
                     
                     # Use cloud URL if available, otherwise local path
                     display_path = cloud_url if cloud_url else f"/data/Documents/{pdf_filename}"
                 else:
                     # Legacy string return
                     pdf_filename = os_module.path.basename(pdf_path)
                     display_path = f"/data/Documents/{pdf_filename}"
